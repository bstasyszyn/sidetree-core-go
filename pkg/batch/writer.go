...
func (r *Writer) main() {
	...
	for {
		select {
		case p := <-r.sendChan:
			log.Infof("[%s] Handling process notification: %v", r.namespace, p)
			pending := r.processAvailable(p.force) > 0
			timer = r.handleTimer(timer, pending)

		case <-timer:
			log.Infof("[%s] Handling batch timeout", r.namespace)
...
func (r *Writer) processAvailable(forceCut bool) uint {
	...
	if pending == 0 || !forceCut {
		log.Infof("[%s] No further processing necessary. Pending operations: %d", r.namespace, pending)
		return pending
	}
...
func (r *Writer) drain() (pending uint, err error) {
	log.Infof("[%s] Draining operations queue...", r.namespace)
	for {
		...
		if n == 0 {
			log.Infof("[%s] ... no more operations to be processed. Pending operations: %d", r.namespace, pending)
			return pending, nil
		}
		log.Infof("[%s] ... processed %d operations. Pending operations: %d", r.namespace, n, pending)
	}
}
...
func (r *Writer) cutAndProcess(forceCut bool) (numProcessed int, pending uint, err error) {
	...
	if len(operations) == 0 {
		log.Infof("[%s] No operations to be processed", r.namespace)
	...
	pending, err = commit()
	if err != nil {
		// FIXME: What to do here? We've already processed the operations but can't remove them from the queue.
		panic(err)
	}
...
func (r *Writer) process(ops []*batch.OperationInfo) error {
	...
	log.Infof("[%s] batch: %s", r.namespace, string(batchBytes))
	...
	log.Infof("[%s] anchor: %s", r.namespace, string(anchorBytes))
