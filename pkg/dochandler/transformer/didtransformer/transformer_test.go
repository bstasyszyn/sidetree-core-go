...
func TestNewTransformer(t *testing.T) {
	...
	const ctx1 = "ctx-1"
	transformer = NewTransformer(WithMethodContext([]string{ctx1}))
	require.Equal(t, 1, len(transformer.methodCtx))
	require.Equal(t, ctx1, transformer.methodCtx[0])

	const ctx2 = "ctx-2"
	transformer = NewTransformer(WithMethodContext([]string{ctx1, ctx2}))
...
func TestWithMethodContext(t *testing.T) {
	...
	transformer := NewTransformer(WithMethodContext([]string{"ctx-1", "ctx-2"}))
