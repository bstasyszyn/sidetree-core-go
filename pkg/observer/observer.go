...
// OperationFilter filters out invalid operations before they are persisted
...
func (p *TxnProcessor) processBatchFile(batchFileAddress string, sidetreeTxn SidetreeTxn) error {
	...
	for suffix, mapping := range mapOperationsByUniqueSuffix(ops) {
		logger.Infof("Filtering operations for namespace [%s] and suffix [%s]", mapping.namespace, suffix)
...
func getNamespace(id string) (string, error) {
...
func mapOperationsByUniqueSuffix(ops []*batch.Operation) map[string]*operationsMapping {
	...
	for _, op := range ops {
		...
		if !ok {
			ns, err := getNamespace(op.ID)
			if err != nil {
				logger.Warnf("Skipping operation since could not get namespace from operation {ID: %s, Type: %s, TransactionTime: %d, TransactionNumber: %d}. Reason: %s", op.ID, op.Type, op.TransactionTime, op.TransactionNumber, err)
				continue
			}
